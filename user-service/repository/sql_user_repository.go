...
func (r *SQLUserRepository) GetSuggestedFriends(ctx context.Context, currentUserID string) ([]*model.User, error) {
	...
	if err != nil || resData == nil {
		logger.Warn("Neo4j friend recommendation query failed, falling back to SQL: %v", err)
		return []*model.User{}, nil
	}

	suggestedIDs := resData.([]string)
	if len(suggestedIDs) == 0 {
		return []*model.User{}, nil
	}

	// Fetch full profile details from SQL
	var users []model.UserEntity
	if err := r.db.WithContext(ctx).Where("id IN ?", suggestedIDs).Find(&users).Error; err != nil {
		return nil, err
	}
	...
	for i, u := range users {
		res[i] = r.entityToModel(&u)
		res[i].MutualFriendsCount = r.countMutualFriends(ctx, currentUserID, u.ID)
	}
