...
func main() {
	...
	authRoutes := r.Group("/v1/auth")
	{
		...
		authRoutes.POST("/refresh-admin", authHandler.RefreshAdmin)
		authRoutes.DELETE("/logout", authHandler.Logout)
		authRoutes.DELETE("/logout-user", authHandler.LogoutUser)
		authRoutes.DELETE("/logout-admin", authHandler.LogoutAdmin)
